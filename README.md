...
- [x] **剪贴板保护** —— 取词前备份文本、图片、文件及富文本等剪贴板格式，约 0.6s 后恢复；期间用户主动复制的新内容不会被覆盖
...
不会。取词瞬间会短暂切换剪贴板内容，约 0.6s 后恢复文本、图片、文件及富文本等常见格式；如果你在这段时间主动复制了新内容，PickTrans 会检测剪贴板变化并跳过恢复，不会覆盖新内容。
</details>
...
设置界面覆盖：服务商预设 / API 地址 / 密钥 / 模型（可在线拉取模型列表）、翻译方向（自动互译或固定目标语言）、划词开关与热键、应用黑名单、OCR 开关与热键、卡片主题与译文字号、开机自启、自动复制译文、更新地址（要求 HTTPS，本机地址可用 HTTP）。
...
- **取词原理**：模拟 `Ctrl+C` 读取剪贴板；取词前通过 Win32 深拷贝可恢复的剪贴板格式，约 0.6s 后按序列号确认未发生新复制再恢复。
